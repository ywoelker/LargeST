...
class BigST(BaseModel):
    ...
    def __init__(self, bigst_args, preprocess_path, preprocess_args):
        super(BigST, self).__init__(node_num=bigst_args['num_nodes'], input_dim=bigst_args['in_dim'], output_dim=1, seq_len=bigst_args['seq_num'], horizon=bigst_args['out_dim'])
    ...
    def forward(self, history_data: torch.Tensor, future_data: torch.Tensor) -> torch.Tensor:
        ...
        if self.use_long:
            ...
            for i in range(history_data.shape[0]):
                with torch.no_grad():
                     feat_sample = self.feat_extractor(history_data[[i],:,:,:], future_data)
