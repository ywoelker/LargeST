...
class STEmbedding(nn.Module):
    ...
    def forward(self, SE, TE, T=288):
        ...
        # temporal embedding (vectorized, correct device/dtype)
        B, S = TE.shape[0], TE.shape[1]
        device, dtype = TE.device, SE.dtype

        dow = (TE[..., 0].long() % 7)                  # [B,S]
        tod = (TE[..., 1].long() % T)                  # [B,S]

        dow_oh = F.one_hot(dow, num_classes=7).to(dtype=dtype, device=device)    # [B,S,7]
        tod_oh = F.one_hot(tod, num_classes=T).to(dtype=dtype, device=device)    # [B,S,T]

        TE = torch.cat([dow_oh, tod_oh], dim=-1).unsqueeze(2)   # [B,S,1,295]
        TE = self.FC_te(TE)                                     # [B,S,1,D]
...
class GMAN(nn.Module):
    ...
    def __init__(self, SE, args, bn_decay):
        ...
        L = args.L; K = args.K; d = args.d; D = K * d
        self.num_his  = args.seq_len
        self.num_pred  = args.horizon
    ...
    def forward(self, X, TE):
        ...
        STE = self.STEmbedding(self.SE, TE)  # [B, num_his+num_pred, N, D]
        expected = self.num_his + self.num_pred
        assert STE.size(1) == expected, f"TE length {STE.size(1)} != num_his+num_pred {expected}"

        STE_his  = STE[:, :self.num_his]
        STE_pred = STE[:, self.num_his:self.num_his + self.num_pred]
        ...
        X = self.FC_2(X)            # [B, num_pred, N, 1]
        return torch.squeeze(X, 3)  # [B, num_pred, N]
