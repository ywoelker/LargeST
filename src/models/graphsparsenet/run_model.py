...
parser.add_argument('--data',type=str,default='/home/kongweiyang/2024/july/BigST-main/data/pems08',help='data path')
...
parser.add_argument('--hid_dim',type=int,default=128,help='')
...
def main():
    ...
    for i in range(1, args.epochs+1):
        ...
        for iter, (x, y, ind) in enumerate(dataloader['train_loader'].get_iterator()):
            # trainx: long history data T=2016 or 864
            trainx = torch.Tensor(x).to(device) # (B, T, N, F)
            # nan_mask = torch.isnan(trainx)  
            # trainx = torch.where(nan_mask, torch.tensor(0, dtype=trainx.dtype).to(trainx.device), trainx) 

            trainx = trainx.transpose(1, 2) # (B, N, T, F)
            trainy = torch.Tensor(y).to(device) # (B, T, N, F)
            # nan_mask = torch.isnan(trainy)  
            # trainy = torch.where(nan_mask, torch.tensor(0, dtype=trainy.dtype).to(trainx.device), trainy) 
        ...
        s1 = time.time()
        for iter, (x, y, ind) in enumerate(dataloader['val_loader'].get_iterator()):
    ...
    s1 = time.time()
    for iter, (x, y, ind) in enumerate(dataloader['test_loader'].get_iterator()):
        testx = torch.Tensor(x).to(device)
        # nan_mask = torch.isnan(trainx)  
        # trainx = torch.where(nan_mask, torch.tensor(0, dtype=trainx.dtype).to(trainx.device), trainx) 
        testx = testx.transpose(1, 2)
