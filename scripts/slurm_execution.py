...
def ablation_repulsion_loss(): 
    ...
    for mask_name in [None, 'tr_drop_050','point_missing_075']:
        model_name = 'DSGNN'
        for repulsion_margin in [0.003, 0.03, 0.3] :
            for repulsion_loss_weight in [0.01, 0.1, 1]:
...
if __name__ == '__main__':
    ...
    for mask_iter in [0,1,2]:
        for mask_name in [None, 'tr_drop_025', 'tr_drop_050', 'tr_drop_075', 'point_missing_050', 'point_missing_075', 'point_missing_095']: 
            ...
            for model_name in ['PDFormer']:#['D2STGNN', 'GMAN', 'PDFormer', 'SPIN', 'OPCR']: #['BigST', 'GSNet', 'AGCRN', 'ASTGCN', 'STGCN', 'DCRNN', 'D2STGNN', 'GMAN', 'GWNET', 'OPCR', 'STGode', 'DSGNN' ,'PDFormer', 'SPIN']:
                gpu_h100 = model_name in ['D2STGNN', 'GMAN', 'DCRNN', 'SPIN', 'PDFormer']
                ...
                create_slurm_job(model_name=model_name, mask_name=mask_name, mask_iter=mask_iter, gpu_h100=gpu_h100, additional_cmd_str=additional_cmd_str + f' --use_metadata True --input_dim {META_DATA_FEATURES[DATASET]} --wandb_tags inference_time_benchmark --max_epochs 1 --train_data_percentage 0.02')
